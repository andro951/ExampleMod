...
namespace clamshellboi.Common.Configs
{
	public class ExampleModConfig : ModConfig
	{
		...
		[Header("$Mods.clamshellboi.Config.ItemHeader")] // Headers are like titles in a config. You only need to declare a header on the item it should appear over, not every item in the category.
		[Label("$Mods.clamshellboi.Config.ExampleWingsToggle.Label")] // A label is the text displayed next to the option. This should usually be a short description of what it does.
		[Tooltip("$Mods.clamshellboi.Config.ExampleWingsToggle.Tooltip")] // A tooltip is a description showed when you hover your mouse over the option. It can be used as a more in-depth explanation of the option.
